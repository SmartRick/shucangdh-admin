 ...
 */
@RestController
@RequestMapping(value = "/scTag")
@Api(value = "",tags = {""})
public class ScTagController extends BaseController {

    @Autowired
    private ScTagService scTagService;

    @ApiOperation(value = "根据Id查询",notes = "@author SmartRick")
    @GetMapping("/{id}")
    public ResponseDTO<ScTagVO> queryById(@PathVariable @ApiParam(value = "id", name = "id", required = true) Long id) {
        return ResponseDTO.succData(scTagService.queryById(id));
    }

    @ApiOperation(value = "分页查询",notes = "@author SmartRick")
    @PostMapping("/page")
    public ResponseDTO<PageResultDTO<ScTagVO>> queryByPage(@RequestBody @ApiParam(value = "分页参数", name = "queryDTO", required = true) ScTagQueryDTO queryDTO) {
        return scTagService.queryByPage(queryDTO);
    }

    @ApiOperation(value = "添加",notes = "@author SmartRick")
    @PostMapping("/")
    public ResponseDTO<String> add(@RequestBody @Validated @ApiParam(value = "", name = "scTag", required = true) ScTagAddDTO addDTO){
        return scTagService.add(addDTO);
    }

    @ApiOperation(value="修改",notes = "@author SmartRick")
    @PutMapping("/")
    public ResponseDTO<String> modify(@RequestBody @Validated @ApiParam(value = "", name = "scTag", required = true) ScTagUpdateDTO updateDTO){
        return scTagService.modify(updateDTO);
    }
    ...
    public ResponseDTO<String> removeByIds(@RequestBody @Validated @ApiParam(value = "ids", name = "ids", required = true) ValidateList<Long> ids) {
        return scTagService.removeByIds(ids);
    }
    ...
    public void batchExport(@RequestBody @Validated @ApiParam(value = "ids", name = "ids", required = true) ValidateList<Long> idList,
        ...
        //查询数据
        List<ScTagExcelVO> scTagList = scTagService.queryBatchExportData(idList);
        //导出操作
        ExportParams ex = new ExportParams("", "Sheet1");
        Workbook workbook = ExcelExportUtil.exportExcel(ex, ScTagExcelVO.class, scTagList);
        downloadExcel("", workbook, response);
    }

    @ApiOperation(value = "导出全部", notes = "@author SmartRick")
    @PostMapping("/export/all")
    public void exportAll(@RequestBody @Validated @ApiParam(value = "分页参数", name = "queryDTO", required = true) ScTagQueryDTO queryDTO,
                          HttpServletResponse response) {
        //查询数据
        List<ScTagExcelVO> scTagList = scTagService.queryAllExportData(queryDTO);
        //导出操作
        ExportParams ex = new ExportParams("", "Sheet1");
        Workbook workbook = ExcelExportUtil.exportExcel(ex, ScTagExcelVO.class, scTagList);
        downloadExcel("", workbook, response);
